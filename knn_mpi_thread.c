...
#ifndef KNN_MAX_THREADS
#define KNN_MAX_THREADS 64
#endif
...
static KNNThreadContext knn_ctx;
static pthread_barrier_t knn_start_barrier;
static pthread_barrier_t knn_done_barrier;

static inline void knn_compute_chunk(int total, int nthreads, int tid,
                                     int *L, int *R) {
    int base = total / nthreads;
    int rem  = total % nthreads;
    int start = tid * base + (tid < rem ? tid : rem);
    int len   = base + (tid < rem ? 1 : 0);
    *L = start;
    *R = start + len;
}

static int knn_clamp_threads(int requested, int local_nq) {
    int threads = requested;
    if (threads < 1) threads = 1;
    if (threads > KNN_MAX_THREADS) threads = KNN_MAX_THREADS;
    if (local_nq == 0) {
        threads = 1;
    } else if (threads > local_nq) {
        threads = local_nq;
    }
    return threads;
}
...
static void parse_args(int argc, char** argv,
                       ...
                       int *nthreads, int *verify_flag) {
    *nq = 128; *npp = 400000; *d = 300; *k = 1024; *nthreads = 1; *verify_flag = 0;
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-v") == 0) { *verify_flag = 1; continue; }
        if (argv[i][0] == '-' && parse_int_kv(argv[i] + 1, "nq", nq)) continue;
        if (parse_int_kv(argv[i], "nq", nq)) continue;
        if (argv[i][0] == '-' && parse_int_kv(argv[i] + 1, "npp", npp)) continue;
        if (parse_int_kv(argv[i], "npp", npp)) continue;
        if (argv[i][0] == '-' && parse_int_kv(argv[i] + 1, "n", npp)) continue;
        if (parse_int_kv(argv[i], "n", npp))   continue; // sinônimo
        if (argv[i][0] == '-' && parse_int_kv(argv[i] + 1, "d", d)) continue;
        if (parse_int_kv(argv[i], "d", d))     continue;
        if (argv[i][0] == '-' && parse_int_kv(argv[i] + 1, "k", k)) continue;
        if (parse_int_kv(argv[i], "k", k))     continue;
        if (argv[i][0] == '-' && parse_int_kv(argv[i] + 1, "t", nthreads)) continue;
        if (parse_int_kv(argv[i], "t", nthreads)) continue;
    }
}
...
static void *knn_thread_worker(void *arg) {
    int tid = *((int*)arg);

    pthread_barrier_wait(&knn_start_barrier);

    int L, R;
    knn_compute_chunk(knn_ctx.local_nq, knn_ctx.nthreads, tid, &L, &R);
    for (int r = L; r < R; r++) {
        const float *q = knn_ctx.Q_local + (long long)r * knn_ctx.d;
        int *out_row = knn_ctx.R_local + (long long)r * knn_ctx.k_eff;
        knn_1query(q, knn_ctx.P, knn_ctx.npp, knn_ctx.d, knn_ctx.k_eff, out_row);
    }

    pthread_barrier_wait(&knn_done_barrier);
    return NULL;
}

static int compute_local_knn_threaded(const float *Q_local, const float *P,
                                      int npp, int d, int k_eff,
                                      int local_nq, int *R_local,
                                      int requested_threads) {
    int threads = knn_clamp_threads(requested_threads, local_nq);

    knn_ctx.Q_local = Q_local;
    knn_ctx.P = P;
    knn_ctx.R_local = R_local;
    knn_ctx.npp = npp;
    knn_ctx.d = d;
    knn_ctx.k_eff = k_eff;
    knn_ctx.local_nq = local_nq;
    knn_ctx.nthreads = threads;

    pthread_barrier_init(&knn_start_barrier, NULL, (unsigned)threads);
    pthread_barrier_init(&knn_done_barrier, NULL, (unsigned)threads);

    pthread_attr_t attr;
    pthread_attr_init(&attr);
    pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);

    pthread_t workers[KNN_MAX_THREADS];
    int tids[KNN_MAX_THREADS];
    ...
    for (int t = 1; t < threads; ++t) {
        tids[t] = t;
        if (pthread_create(&workers[t], &attr, knn_thread_worker, &tids[t]) != 0) {
            fprintf(stderr, "Rank %d: Falha ao criar thread %d\n", rank, t);
    ...
    tids[0] = 0;
    (void)knn_thread_worker(&tids[0]);

    pthread_barrier_destroy(&knn_done_barrier);
    pthread_barrier_destroy(&knn_start_barrier);
...
int main(int argc, char** argv) {
    ...
    int nq, npp, d, k, nthreads_req, verify_flag;
    parse_args(argc, argv, &nq, &npp, &d, &k, &nthreads_req, &verify_flag);
    ...
    int threads_eff = knn_clamp_threads(nthreads_req, local_nq);

    if (rank == 0) {
        printf("Params: nq=%d npp=%d d=%d k=%d  procs=%d  threads/rank=%d verify=%s\n",
               nq, npp, d, k, nprocs, threads_eff, verify_flag ? "on" : "off");
        fflush(stdout);
    }
    ...
    compute_local_knn_threaded(Q_local, P, npp, d, k_eff, local_nq,
                               R_local, nthreads_req);
    ...
    if (rank == 0) {
        ...
        printf("  Compute:    %.6f s (threads/rank=%d)\n", t_comp, threads_eff);
